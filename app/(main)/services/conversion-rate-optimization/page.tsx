...
const services = [
  { icon: "🧪", title: "A/B & Multivariate Testing", description: "Scientific testing of headlines, CTAs, layouts, images, forms, and copy to identify what converts best — eliminating guesswork with statistical confidence." },
  { icon: "📄", title: "Landing Page Optimization", description: "Design, test, and optimize high-converting landing pages for listings, lead magnets, home valuations, and buyer/seller funnels that maximize form submissions." },
  { icon: "🔄", title: "Funnel Analysis & Optimization", description: "Map and analyze every step of your conversion funnel to identify where leads drop off — then implement targeted fixes to plug the leaks." },
  { icon: "📱", title: "UX & Mobile Optimization", description: "Improve site speed, mobile responsiveness, navigation, and user flow to reduce friction and make it effortless for visitors to become leads." },
  { icon: "📝", title: "Form & CTA Optimization", description: "Optimize form fields, button copy, placement, and design to dramatically increase submission rates without sacrificing lead quality." },
  { icon: "🔥", title: "Heatmap & Behavior Analysis", description: "Use heatmaps, session recordings, and scroll tracking to understand exactly how visitors interact with your pages — then optimize based on real behavior." },
];
...
export default function CROPage() {
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* Hero */}
      <section className="relative overflow-hidden bg-gradient-to-br from-lime-600 via-green-700 to-lime-900 pt-32 pb-28">
        <div className="absolute inset-0 opacity-10" style={{ backgroundImage: "url(\"data:image/svg+xml,%3Csvg width='60' height='60' viewBox='0 0 60 60' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='none' fill-rule='evenodd'%3E%3Cg fill='%23ffffff' fill-opacity='0.4'%3E%3Cpath d='M36 34v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2V6h4V4h-4zM6 34v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6 4V0H4v4H0v2h4v4h2V6h4V4H6z'/%3E%3C/g%3E%3C/g%3E%3C/svg%3E\")" }} />
        <div className="pointer-events-none absolute -top-24 -left-24 h-96 w-96 rounded-full bg-lime-400/20 blur-3xl" />
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <nav className="flex justify-center mb-6 text-lime-200 text-sm">
          ...
          <p className="text-xl text-lime-100 max-w-3xl mx-auto mb-10 leading-relaxed">A/B testing, landing page optimization, and funnel analysis that increase your conversion rates — generating more leads without spending more on traffic.</p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link href="/contact" className="bg-white text-lime-700 px-8 py-4 rounded-xl font-semibold text-lg hover:bg-lime-50 transition-all shadow-lg shadow-lime-900/20 hover:shadow-xl hover:-translate-y-0.5">Get Your Free CRO Audit</Link>
          ...
          <p className="mt-6 text-lime-200/70 text-sm">2,400+ A/B tests run for real estate clients</p>
        </div>
      </section>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
          {stats.map((stat, i) => (
            <div key={i} className="bg-white rounded-2xl p-6 text-center shadow-lg shadow-gray-200/50 border border-gray-100 hover:shadow-xl transition-shadow">
              <div className="text-3xl md:text-4xl font-bold text-lime-600 mb-1">{stat.value}</div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-16 pb-4">
        <div className="flex flex-wrap items-center justify-center gap-x-8 gap-y-3 text-sm text-gray-400">
          {["Statistical Rigor", "Real Estate–Specific Tests", "Heatmap & Behavior Analysis", "Continuous Optimization"].map((item, i) => (
            <span key={i} className="flex items-center gap-2"><svg className="w-5 h-5 text-lime-500" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.857-9.809a.75.75 0 00-1.214-.882l-3.483 4.79-1.88-1.88a.75.75 0 10-1.06 1.061l2.5 2.5a.75.75 0 001.137-.089l4-5.5z" clipRule="evenodd" /></svg> {item}</span>
          ))}
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        <div className="text-center mb-16">
          <span className="text-lime-600 font-semibold text-sm uppercase tracking-wider">Our Services</span>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {services.map((s, i) => (
            <div key={i} className="group bg-white border border-gray-100 rounded-2xl p-8 hover:border-lime-200 hover:shadow-lg hover:shadow-lime-100/50 transition-all duration-300">
              <div className="w-14 h-14 bg-gradient-to-br from-lime-50 to-green-100 rounded-2xl flex items-center justify-center text-2xl mb-5 group-hover:scale-110 transition-transform duration-300">{s.icon}</div>
      ...
      <section className="bg-gradient-to-br from-gray-900 to-gray-800 py-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <span className="text-lime-400 font-semibold text-sm uppercase tracking-wider">Proven Results</span>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
            {results.map((r, i) => (
              <div key={i} className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6 text-center hover:bg-white/10 transition-colors">
                <div className="text-4xl font-bold text-lime-400 mb-2">{r.metric}</div>
      ...
      <section className="py-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <span className="text-lime-600 font-semibold text-sm uppercase tracking-wider">Why CRO</span>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {benefits.map((b, i) => (
              <div key={i} className="bg-lime-50/50 border border-lime-100 rounded-2xl p-6 hover:bg-lime-50 transition-colors">
                <div className="flex items-center gap-3 mb-3">
                  <div className="w-8 h-8 bg-lime-600 rounded-lg flex items-center justify-center flex-shrink-0">
      ...
      <section className="bg-gray-50 border-y border-gray-100 py-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <span className="text-lime-600 font-semibold text-sm uppercase tracking-wider">Testimonials</span>
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {testimonials.map((t, i) => (
              <div key={i} className="bg-white rounded-2xl p-8 shadow-sm border border-gray-100 relative">
                <svg className="absolute top-6 right-8 w-12 h-12 text-lime-100" fill="currentColor" viewBox="0 0 24 24"><path d="M14.017 21v-7.391c0-5.704 3.731-9.57 8.983-10.609l.995 2.151c-2.432.917-3.995 3.638-3.995 5.849h4v10H14.017zM0 21v-7.391c0-5.704 3.731-9.57 8.983-10.609L9.978 5.151c-2.432.917-3.995 3.638-3.995 5.849h4v10H0z" /></svg>
                ...
                <div><div className="font-bold text-gray-900">{t.author}</div><div className="text-lime-600 text-sm">{t.role}</div></div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24">
        <div className="text-center mb-16">
          <span className="text-lime-600 font-semibold text-sm uppercase tracking-wider">Our Process</span>
        ...
        <div className="relative">
          <div className="hidden md:block absolute top-6 left-[10%] right-[10%] h-0.5 bg-gradient-to-r from-lime-200 via-lime-400 to-lime-200" />
          <div className="grid md:grid-cols-5 gap-8">
            {steps.map((s) => (
              <div key={s.step} className="text-center relative">
                <div className="w-12 h-12 bg-lime-600 text-white rounded-full flex items-center justify-center text-lg font-bold mx-auto mb-4 relative z-10 shadow-lg shadow-lime-200">{s.step}</div>
      ...
      <section className="bg-gray-50 border-y border-gray-100 py-24">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <span className="text-lime-600 font-semibold text-sm uppercase tracking-wider">FAQ</span>
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mt-3">Common Questions About CRO</h2>
          </div>
          <div className="space-y-4">
            {faqs.map((faq, i) => (
              <details key={i} className="group bg-white rounded-2xl border border-gray-100 hover:border-lime-200 transition-colors">
      ...
      <section className="py-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <span className="text-lime-600 font-semibold text-sm uppercase tracking-wider">Related Services</span>
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mt-3">Maximize Your CRO Results</h2>
          </div>
          <div className="grid md:grid-cols-3 gap-6">
            {relatedServices.map((rs, i) => (
              <Link key={i} href={rs.href} className="group bg-white border border-gray-100 rounded-2xl p-8 hover:border-lime-200 hover:shadow-lg hover:shadow-lime-50 transition-all block">
                <h3 className="text-xl font-bold text-gray-900 mb-3 group-hover:text-lime-600 transition-colors">{rs.title}</h3>
                <p className="text-gray-500 leading-relaxed mb-4">{rs.description}</p>
                <span className="text-lime-600 font-semibold text-sm inline-flex items-center gap-1 group-hover:gap-2 transition-all">Learn more <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg></span>
              </Link>
            ))}
          </div>
        </div>
      ...
      {/* CTA */}
      <section className="relative overflow-hidden bg-gradient-to-br from-lime-600 via-green-700 to-lime-900 py-24">
        <div className="pointer-events-none absolute -top-24 -right-24 h-96 w-96 rounded-full bg-lime-400/20 blur-3xl" />
        ...
        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-xl text-lime-100 mb-10 max-w-2xl mx-auto leading-relaxed">Book a free conversion audit and discover exactly where your website is leaking leads — and how to fix it for more closings with the same traffic.</p>
          <Link href="/contact" className="inline-block bg-white text-lime-700 px-8 py-4 rounded-xl font-semibold text-lg hover:bg-lime-50 transition-all shadow-lg shadow-lime-900/20 hover:shadow-xl hover:-translate-y-0.5">Get Your Free CRO Audit</Link>
          <p className="mt-6 text-lime-200/60 text-sm">Includes heatmap analysis and prioritized optimization roadmap</p>
        </div>
      </section>
